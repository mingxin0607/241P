...
// dfs from point city, mark all cities it could visit as visited
void dfs(City* city, vector<string> cities_lst, set<string>& to_be_visit) {
    // cout << "---1---";
    queue<City*> queue;
    queue.push(city);
    ...
    while (!queue.empty()) {
        ...
        for (City* neighbor : currentCity->connections) {
            if (to_be_visit.find(neighbor->name) != to_be_visit.end()) {
                to_be_visit.erase(neighbor->name);
                queue.push(neighbor);
            }
        }
    }
...
int count_islands(map<string, City*>& cities, vector<string> cities_lst) {
    set<string> to_be_visit;
    for (auto& city : cities_lst){
        to_be_visit.insert(city);
    }
    int islands = 0;

    for (auto& city_name : cities_lst) {
        
        if (to_be_visit.find(city_name) != to_be_visit.end()) {
            City* city = cities[city_name];
            to_be_visit.erase(city_name);
            dfs(city, cities_lst, to_be_visit);
            islands++;
        }
    }
...
// dfs to calculate population on an island
void dfs2(City* city, vector<string> cities_lst, std::set<string>& to_be_visit, int& population) {
...
map<string, int> get_island_populations(map<string, City*>& cities, vector<string> cities_lst) {
    set<string> to_be_visit;
    for (auto& city : cities_lst){
        to_be_visit.insert(city);
    }
    ...
    for (auto& city_name : cities_lst) {
        ...
        if (to_be_visit.find(city_name) != to_be_visit.end()) {
            ...
            City* city = cities[city_name];
            to_be_visit.erase(city_name);
            dfs2(city, cities_lst, to_be_visit, population);
...
void test4(){
    ...
    int n_island = count_islands(cities, cities_lst);
    cout << "number of islands is " << n_island << endl;
    map<string,int> populations = get_island_populations(cities, cities_lst);
